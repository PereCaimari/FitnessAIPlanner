...
function WorkoutDetail({ workout, onClose, onSave, onDelete }: { workout: Workout; onClose: () => void; onSave: (workout: Workout) => void; onDelete: (id: string) => void }) {
  ...
  const saveChanges = () => {
    ...
    onSave({ ...workout, title: title.trim(), type, date: /^\d{4}-\d{2}-\d{2}$/.test(date) ? date : workout.date, duration: duration.trim() ? `${duration.trim()} min` : '—', distance: distance.trim() || undefined, pace: pace.trim() || undefined, exercises, gpxSplits, rpe: Math.min(10, Math.max(1, Number(rpe) || 1)) })
    setEditing(false)
  }
