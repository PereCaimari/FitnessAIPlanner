...
function App() {
  ...
  const updateWorkout = async (updated: Workout) => {
    try {
      ...
      const paceSeconds = paceParts.length === 2 && paceParts.every(Number.isFinite) ? Math.round(paceParts[0] * 60 + paceParts[1]) : null
      const { error } = await supabase.from('workouts').update({ title: updated.title, type: updated.type, workout_date: updated.date, duration_minutes: Math.max(0, Math.round(Number.parseFloat(updated.duration) || 0)), distance_km: updated.distance && Number.isFinite(Number(updated.distance)) ? Number(updated.distance) : null, average_pace_seconds: paceSeconds, intensity: `RPE ${updated.rpe}`, gpx_splits: updated.gpxSplits ?? null }).eq('id', updated.id)
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {authLoading || workoutsLoading ? <div className="loading-state">Cargando tus entrenamientos…</div> : section === 'summary' && <Summary workouts={workouts} onHistory={() => setSection('history')} onSelectWorkout={setSelectedWorkout} calendarMode={calendarMode} setCalendarMode={setCalendarMode} />}
...
function WorkoutDetail({ workout, onClose, onSave, onDelete }: { workout: Workout; onClose: () => void; onSave: (workout: Workout) => void; onDelete: (id: string) => void }) {
