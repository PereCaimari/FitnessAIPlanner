...
function WorkoutModal({ catalog, groups, newTitle, setNewTitle, newType, setNewType, selectedExercises, setSelectedExercises, search, setSearch, group, setGroup, runningMode, setRunningMode, runningDistance, setRunningDistance, runningTime, setRunningTime, realPace, setRealPace, gpxFileName, setGpxFileName, targetPace, setTargetPace, runningBlocks, setRunningBlocks, onSave, onClose }: { catalog: Exercise[]; groups: string[]; newTitle: string; setNewTitle: (v: string) => void; newType: string; setNewType: (v: string) => void; selectedExercises: Exercise[]; setSelectedExercises: (v: Exercise[]) => void; search: string; setSearch: (v: string) => void; group: string; setGroup: (v: string) => void; runningMode: 'Rodaje' | 'Series'; setRunningMode: (v: 'Rodaje' | 'Series') => void; runningDistance: string; setRunningDistance: (v: string) => void; runningTime: string; setRunningTime: (v: string) => void; realPace: string; setRealPace: (v: string) => void; gpxFileName: string; setGpxFileName: (v: string) => void; targetPace: string; setTargetPace: (v: string) => void; runningBlocks: RunningBlock[]; setRunningBlocks: (v: RunningBlock[]) => void; onSave: () => void; onClose: () => void }) {
  const parseGpx = async (file: File) => {
    ...
    const distanceKm = points.slice(1).reduce((total, point, index) => { const previous = points[index]; const dLat = toRadians(point.lat - previous.lat); const dLon = toRadians(point.lon - previous.lon); const a = Math.sin(dLat / 2) ** 2 + Math.cos(toRadians(previous.lat)) * Math.cos(toRadians(point.lat)) * Math.sin(dLon / 2) ** 2; return total + 6371 * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a)) }, 0)
