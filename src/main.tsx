...
function Planner({ answers, setAnswers, step, setStep, onGenerate, plan }: { answers: PlannerAnswers; setAnswers: React.Dispatch<React.SetStateAction<PlannerAnswers>>; step: number; setStep: (step: number) => void; onGenerate: () => void; plan: string }) {
  ...
  return <section className="planner-card planner-wizard">
    ...
    <div className="planner-form">
      ...
      {step === 3 && <><p className="wizard-kicker">PASO 3 DE 4</p><h3>Personalicemos el plan</h3>{answers.modalities.includes('Running') && <div className="wizard-fields"><label>Distancia objetivo<input value={answers.runningDistance} onChange={event => update('runningDistance', event.target.value)} placeholder="10 km" /></label><label>Ritmo habitual<input value={answers.runningPace} onChange={event => update('runningPace', event.target.value)} placeholder="5:30 min/km" /></label></div>}{answers.modalities.includes('Natación') && <div className="wizard-fields"><label>Distancia habitual<input value={answers.swimmingDistance} onChange={event => update('swimmingDistance', event.target.value)} placeholder="1500 m" /></label><label>Estilo principal<select value={answers.swimmingStrokes} onChange={event => update('swimmingStrokes', event.target.value)}><option>Crol</option><option>Espalda</option><option>Braza</option><option>Mariposa</option><option>Varios</option></select></label></div>}{answers.modalities.includes('Gimnasio') && <label>Material disponible<textarea value={answers.equipment} onChange={event => update('equipment', event.target.value)} placeholder="Ej. Gimnasio completo, mancuernas, bandas..." /></label>}<label>Lesiones, limitaciones o preferencias<textarea value={answers.limitations} onChange={event => update('limitations', event.target.value)} placeholder="Cuéntanos qué debemos tener en cuenta" /></label></>}
