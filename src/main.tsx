...
function RpeChart({ workouts }: { workouts: Workout[] }) { const recent = workouts.slice().sort((a, b) => a.date.localeCompare(b.date)).slice(-8); const width = 760; const height = 210; const padX = 42; const padY = 28; const chartWidth = width - padX * 2; const chartHeight = height - padY * 2; const points = recent.map((workout, index) => `${padX + (recent.length === 1 ? chartWidth / 2 : index * chartWidth / (recent.length - 1))},${padY + chartHeight - (Math.max(1, Math.min(10, workout.rpe)) - 1) * chartHeight / 9}`).join(' '); return <section className="rpe-chart-card content-section"><div className="section-heading"><div><p className="eyebrow">INTENSIDAD PERCIBIDA</p><h2>Últimos RPE</h2></div><span className="chart-caption">Escala de esfuerzo · 1–10</span></div>{recent.length ? <div className="rpe-chart-wrap"><svg className="rpe-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Gráfico de los últimos valores RPE"><line className="chart-axis" x1={padX} y1={padY} x2={padX} y2={height - padY} /><line className="chart-axis" x1={padX} y1={height - padY} x2={width - padX} y2={height - padY} />{[1, 5, 10].map(value => { const y = padY + chartHeight - (value - 1) * chartHeight / 9; return <g key={value}><line className="chart-grid-line" x1={padX} y1={y} x2={width - padX} y2={y} /><text className="chart-label" x="14" y={y + 4}>{value}</text></g> })}<polyline className="rpe-line" points={points} />{recent.map((workout, index) => { const x = padX + (recent.length === 1 ? chartWidth / 2 : index * chartWidth / (recent.length - 1)); const y = padY + chartHeight - (Math.max(1, Math.min(10, workout.rpe)) - 1) * chartHeight / 9; return <g key={workout.id}><circle className="rpe-point" cx={x} cy={y} r="5" /><text className="chart-value" x={x} y={y - 11}>{workout.rpe}</text><text className="chart-date" x={x} y={height - 8} textAnchor="middle">{workout.date.slice(5)}</text></g> })}</svg></div> : <p className="chart-empty">Registra entrenamientos con RPE para ver tu evolución.</p>}</section> }
