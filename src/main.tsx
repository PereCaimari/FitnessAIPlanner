...
function App() {
  ...
  const [plannerAnswers, setPlannerAnswers] = useState<PlannerAnswers>({ goal: '', modalities: ['Gimnasio'], level: 'Principiante', sessionsPerWeek: '3', sessionMinutes: '45', planWeeks: '4', availability: '', limitations: '', runningDistance: '10 km', runningPace: '', swimmingDistance: '1500 m', swimmingStrokes: 'Crol', equipment: 'Gimnasio completo' })
...
function Planner({ answers, setAnswers, step, setStep, onGenerate, onSavePlan, plan, planSaving, planSaved }: { answers: PlannerAnswers; setAnswers: React.Dispatch<React.SetStateAction<PlannerAnswers>>; step: number; setStep: (step: number) => void; onGenerate: () => void; onSavePlan: () => void; plan: AiPlanResponse | null; planSaving: boolean; planSaved: boolean }) {
  ...
  return <section className="planner-card planner-wizard">
    ...
    <div className="planner-form">
      ...
      {step === 2 && <><p className="wizard-kicker">PASO 2 DE 4</p><h3>¿Cuándo puedes entrenar?</h3><div className="wizard-fields"><label>Sesiones por semana<select value={answers.sessionsPerWeek} onChange={event => update('sessionsPerWeek', event.target.value)}><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option></select></label><label>Duración por sesión<select value={answers.sessionMinutes} onChange={event => update('sessionMinutes', event.target.value)}><option>30</option><option>45</option><option>60</option><option>75</option><option>90</option></select></label></div><label>Duración del plan<select value={answers.planWeeks} onChange={event => update('planWeeks', event.target.value)}><option>4</option><option>6</option><option>8</option><option>12</option></select></label><label>Días o momentos preferidos<textarea value={answers.availability} onChange={event => update('availability', event.target.value)} placeholder="Ej. Lunes, miércoles y sábado por la mañana" /></label></>}
