...
type Workout = { id: number; title: string; type: string; date: string; duration: string; gpxSplits?: string[] }
...
type GpxSummary = { points: number; elevationGain: number; elevationLoss: number; startTime: string; endTime: string; splits: string[] }
...
function App() {
  ...
  const addWorkout = () => {
    ...
    setWorkouts([{ id: Date.now(), title: `${newTitle}${details}`, type: newType, date: 'Ahora', duration: newType === 'Running' && runningTime ? `${runningTime} min` : '—', gpxSplits: gpxSummary?.splits }, ...workouts])
    setNewTitle('')
    setShowForm(false)
...
function WorkoutList({ workouts }: { workouts: Workout[] }) { return <div className="workout-list">{workouts.map(workout => <div className="workout-row" key={workout.id}><div className="workout-icon">{workout.type === 'Running' ? '⌁' : workout.type === 'Gimnasio' ? '↗' : '○'}</div><div className="workout-info"><strong>{workout.title}</strong><span>{workout.type} · {workout.date}</span>{workout.gpxSplits?.length ? <div className="saved-splits"><small>Ritmo por km</small>{workout.gpxSplits.map(split => <span key={split}>{split}</span>)}</div> : null}</div><b>{workout.duration}</b><span className="row-arrow">→</span></div>)}</div> }
...
function WorkoutModal({ catalog, groups, newTitle, setNewTitle, newType, setNewType, selectedExercises, setSelectedExercises, search, setSearch, group, setGroup, runningMode, setRunningMode, runningDistance, setRunningDistance, runningTime, setRunningTime, realPace, setRealPace, gpxFileName, setGpxFileName, gpxSummary, setGpxSummary, targetPace, setTargetPace, runningBlocks, setRunningBlocks, onSave, onClose }: { catalog: Exercise[]; groups: string[]; newTitle: string; setNewTitle: (v: string) => void; newType: string; setNewType: (v: string) => void; selectedExercises: Exercise[]; setSelectedExercises: (v: Exercise[]) => void; search: string; setSearch: (v: string) => void; group: string; setGroup: (v: string) => void; runningMode: 'Rodaje' | 'Series'; setRunningMode: (v: 'Rodaje' | 'Series') => void; runningDistance: string; setRunningDistance: (v: string) => void; runningTime: string; setRunningTime: (v: string) => void; realPace: string; setRealPace: (v: string) => void; gpxFileName: string; setGpxFileName: (v: string) => void; gpxSummary: GpxSummary | null; setGpxSummary: (v: GpxSummary | null) => void; targetPace: string; setTargetPace: (v: string) => void; runningBlocks: RunningBlock[]; setRunningBlocks: (v: RunningBlock[]) => void; onSave: () => void; onClose: () => void }) {
  const parseGpx = async (file: File) => {
    ...
    setGpxSummary({ points: points.length, elevationGain: Math.round(elevationGain), elevationLoss: Math.round(elevationLoss), startTime, endTime, splits: splitTimes })
