...
function Calendar({ workouts, mode, setMode, onSelectWorkout }: { workouts: Workout[]; mode: CalendarMode; setMode: (mode: CalendarMode) => void; onSelectWorkout: (workout: Workout) => void }) { const today = new Date(); const year = today.getFullYear(); const month = today.getMonth(); const daysInMonth = new Date(year, month + 1, 0).getDate(); const firstDay = (new Date(year, month, 1).getDay() + 6) % 7; const dayWorkouts = (day: number) => workouts.filter(workout => (workout.date.startsWith('Hoy') && day === today.getDate()) || (workout.date.startsWith('Ayer') && day === today.getDate() - 1) || (workout.date.startsWith('12 ago') && day === 12 && month === 7)); const weekDays = Array.from({ length: 7 }, (_, index) => { const date = new Date(year, month, today.getDate() - ((today.getDay() + 6) % 7) + index); return date.getDate() }); const days = mode === 'week' ? weekDays : Array.from({ length: daysInMonth }, (_, index) => index + 1); return <section className="calendar-card content-section"><div className="section-heading"><div><p className="eyebrow">TU ACTIVIDAD</p><h2>Calendario de entrenamientos</h2></div><div className="calendar-switch"><button className={mode === 'week' ? 'active' : ''} onClick={() => setMode('week')}>Semanal</button><button className={mode === 'month' ? 'active' : ''} onClick={() => setMode('month')}>Mensual</button></div></div><div className={`calendar-grid ${mode === 'month' ? 'calendar-month' : ''}`}>{(mode === 'month' ? ['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'] : ['Lun','Mar','Mié','Jue','Vie','Sáb','Dom']).map(day => <span className="calendar-weekday" key={day}>{day}</span>)}{mode === 'month' && Array.from({ length: firstDay }, (_, index) => <span className="calendar-empty" key={`empty-${index}`} />)}{days.map(day => { const sessions = dayWorkouts(day); return <button className={`calendar-day ${sessions.length ? 'has-workout' : ''} ${day === today.getDate() ? 'today' : ''}`} key={day} onClick={() => sessions[0] && onSelectWorkout(sessions[0])}><strong>{day}</strong>{sessions.map(session => <small key={session.id}>{session.type === 'Running' ? '⌁' : '↗'} {session.title.split(' ')[0]}</small>)}</button> })}</div><p className="calendar-hint">Pulsa en un día con actividad para ver lo que hiciste.</p></section> }
...
function WorkoutDetail({ workout, onClose }: { workout: Workout; onClose: () => void }) { return <div className="modal-backdrop" onClick={onClose}><div className="modal workout-detail" onClick={event => event.stopPropagation()}><div className="modal-heading"><div><p className="eyebrow">SESIÓN DEL {workout.date.toUpperCase()}</p><h2>{workout.title}</h2></div><button className="close" onClick={onClose}>×</button></div><div className="detail-grid"><div><small>TIPO</small><strong>{workout.type}</strong></div><div><small>DURACIÓN</small><strong>{workout.duration}</strong></div><div><small>RPE</small><strong>{workout.rpe}/10</strong></div></div>{workout.gpxSplits?.length ? <div className="saved-splits detail-splits"><small>Ritmo por km</small>{workout.gpxSplits.map(split => <span key={split}>{split}</span>)}</div> : <p className="detail-empty">No hay detalles adicionales guardados para esta sesión.</p>}<button className="primary-button full" onClick={onClose}>Cerrar</button></div></div> }
