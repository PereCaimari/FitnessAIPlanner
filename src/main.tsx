...
function App() {
  ...
  return <div className="app-shell">
    ...
    {showForm && <WorkoutModal catalog={catalog} groups={muscleGroups} newTitle={newTitle} setNewTitle={setNewTitle} newType={newType} setNewType={setNewType} selectedExercises={selectedExercises} setSelectedExercises={setSelectedExercises} search={exerciseSearch} setSearch={setExerciseSearch} group={exerciseGroup} setGroup={setExerciseGroup} runningMode={runningMode} setRunningMode={setRunningMode} runningDistance={runningDistance} setRunningDistance={setRunningDistance} runningTime={runningTime} setRunningTime={setRunningTime} targetPace={targetPace} setTargetPace={setTargetPace} runningBlocks={runningBlocks} setRunningBlocks={setRunningBlocks} onSave={addWorkout} onClose={() => setShowForm(false)} />}
  </div>
}
...
function WorkoutModal({ catalog, groups, newTitle, setNewTitle, newType, setNewType, selectedExercises, setSelectedExercises, search, setSearch, group, setGroup, runningMode, setRunningMode, runningDistance, setRunningDistance, runningTime, setRunningTime, targetPace, setTargetPace, runningBlocks, setRunningBlocks, onSave, onClose }: { catalog: Exercise[]; groups: string[]; newTitle: string; setNewTitle: (v: string) => void; newType: string; setNewType: (v: string) => void; selectedExercises: Exercise[]; setSelectedExercises: (v: Exercise[]) => void; search: string; setSearch: (v: string) => void; group: string; setGroup: (v: string) => void; runningMode: 'Rodaje' | 'Series'; setRunningMode: (v: 'Rodaje' | 'Series') => void; runningDistance: string; setRunningDistance: (v: string) => void; runningTime: string; setRunningTime: (v: string) => void; targetPace: string; setTargetPace: (v: string) => void; runningBlocks: RunningBlock[]; setRunningBlocks: (v: RunningBlock[]) => void; onSave: () => void; onClose: () => void }) {
  ...
  const totalMinutes = runningTime ? (() => { const parts = runningTime.split(':').map(Number); return parts.length === 2 ? parts[0] + parts[1] / 60 : Number(runningTime) })() : 0
  const paceValue = totalMinutes > 0 && Number(runningDistance) > 0 ? totalMinutes / Number(runningDistance) : 0
  const averagePace = paceValue ? `${Math.floor(paceValue)}:${String(Math.round((paceValue % 1) * 60)).padStart(2, '0')} min/km` : '—'
