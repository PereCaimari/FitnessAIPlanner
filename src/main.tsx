...
function App() {
  ...
  const [plannerAnswers, setPlannerAnswers] = useState<PlannerAnswers>({ goal: '', modality: 'Gimnasio', level: 'Principiante', sessionsPerWeek: '3', sessionMinutes: '45', planWeeks: '4', availability: '', limitations: '', runningDistance: '10 km', runningPace: '', swimmingDistance: '1500 m', swimmingStrokes: 'Crol', equipment: 'Gimnasio completo' })
  ...
  const generatePlan = () => {
    ...
    setPlan(`Cuestionario completado para ${plannerAnswers.modality}. El siguiente paso será generar tu plan personalizado con IA.`)
    setPlannerStep(4)
  }
...
function Planner({ answers, setAnswers, step, setStep, onGenerate, plan }: { answers: PlannerAnswers; setAnswers: React.Dispatch<React.SetStateAction<PlannerAnswers>>; step: number; setStep: (step: number) => void; onGenerate: () => void; plan: string }) {
  const update = (key: keyof PlannerAnswers, value: string) => setAnswers(current => ({ ...current, [key]: value }))
  const canContinue = step === 1 ? answers.goal.trim().length > 0 : true
  ...
  return <section className="planner-card planner-wizard">
    ...
    <div className="planner-form">
      {step === 1 && <><p className="wizard-kicker">PASO 1 DE 4</p><h3>¿Qué quieres conseguir?</h3><label>Tu objetivo<textarea value={answers.goal} onChange={event => update('goal', event.target.value)} placeholder="Ej. Prepararme para una carrera de 10 km o ganar fuerza" /></label><label>Modalidad principal<select value={answers.modality} onChange={event => update('modality', event.target.value as PlannerModality)}><option>Gimnasio</option><option>Running</option><option>Natación</option></select></label><label>Nivel de experiencia<select value={answers.level} onChange={event => update('level', event.target.value)}><option>Principiante</option><option>Intermedio</option><option>Avanzado</option></select></label></>}
      ...
      {step === 3 && <><p className="wizard-kicker">PASO 3 DE 4</p><h3>Personalicemos el plan</h3>{answers.modality === 'Running' && <div className="wizard-fields"><label>Distancia objetivo<input value={answers.runningDistance} onChange={event => update('runningDistance', event.target.value)} placeholder="10 km" /></label><label>Ritmo habitual<input value={answers.runningPace} onChange={event => update('runningPace', event.target.value)} placeholder="5:30 min/km" /></label></div>}{answers.modality === 'Natación' && <div className="wizard-fields"><label>Distancia habitual<input value={answers.swimmingDistance} onChange={event => update('swimmingDistance', event.target.value)} placeholder="1500 m" /></label><label>Estilo principal<select value={answers.swimmingStrokes} onChange={event => update('swimmingStrokes', event.target.value)}><option>Crol</option><option>Espalda</option><option>Braza</option><option>Mariposa</option><option>Varios</option></select></label></div>}{answers.modality === 'Gimnasio' && <label>Material disponible<textarea value={answers.equipment} onChange={event => update('equipment', event.target.value)} placeholder="Ej. Gimnasio completo, mancuernas, bandas..." /></label>}<label>Lesiones, limitaciones o preferencias<textarea value={answers.limitations} onChange={event => update('limitations', event.target.value)} placeholder="Cuéntanos qué debemos tener en cuenta" /></label></>}
      {step === 4 && <><p className="wizard-kicker">PASO 4 DE 4</p><h3>Todo listo para crear tu plan</h3><div className="answer-summary"><div><small>OBJETIVO</small><strong>{answers.goal}</strong></div><div><small>MODALIDAD</small><strong>{answers.modality} · {answers.level}</strong></div><div><small>FRECUENCIA</small><strong>{answers.sessionsPerWeek} sesiones · {answers.sessionMinutes} min</strong></div><div><small>DURACIÓN</small><strong>{answers.planWeeks} semanas</strong></div></div><p className="planner-note">Usaremos estas respuestas para generar sesiones programadas que después podrás revisar y modificar.</p>{plan && <div className="plan-result"><strong>Base guardada</strong><p>{plan}</p></div>}</>}
