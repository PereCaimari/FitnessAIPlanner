...
function WorkoutDetail({ workout, onClose, onSave, onDelete }: { workout: Workout; onClose: () => void; onSave: (workout: Workout) => void; onDelete: (id: number) => void }) {
  ...
  const saveChanges = () => {
    ...
    onSave({ ...workout, title: title.trim(), type, rpe: Number(rpe) })
    setEditing(false)
  }
  const removeWorkout = () => {
    if (window.confirm('¿Quieres eliminar este entrenamiento? Esta acción no se puede deshacer.')) onDelete(workout.id)
  }
  return <div className="modal-backdrop" onClick={onClose}><div className="modal workout-detail" onClick={event => event.stopPropagation()}><div className="modal-heading"><div><p className="eyebrow">SESIÓN DEL {workout.date.toUpperCase()}</p>{editing ? <input value={title} onChange={event => setTitle(event.target.value)} aria-label="Nombre del entrenamiento" /> : <h2>{workout.title}</h2>}</div><button className="close" onClick={onClose}>×</button></div>{editing ? <><label>Tipo<select value={type} onChange={event => setType(event.target.value)}><option>Gimnasio</option><option>Running</option><option>Ciclismo</option><option>Personalizado</option></select></label><label>RPE (1–10)<input type="number" min="1" max="10" value={rpe} onChange={event => setRpe(String(Math.min(10, Math.max(1, Number(event.target.value) || 1))))} /></label></> : <div className="detail-grid"><div><small>TIPO</small><strong>{workout.type}</strong></div><div><small>DURACIÓN</small><strong>{workout.duration}</strong></div><div><small>RPE</small><strong>{workout.rpe}/10</strong></div></div>}{!editing && (workout.gpxSplits?.length ? <div className="saved-splits detail-splits"><small>Ritmo por km</small>{workout.gpxSplits.map(split => <span key={split}>{split}</span>)}</div> : <p className="detail-empty">No hay detalles adicionales guardados para esta sesión.</p>)}{editing ? <div className="detail-actions"><button className="secondary-button" onClick={() => setEditing(false)}>Cancelar</button><button className="primary-button" onClick={saveChanges}>Guardar cambios</button></div> : <div className="detail-actions"><button className="secondary-button" onClick={() => setEditing(true)}>Editar entrenamiento</button><button className="danger-button" onClick={removeWorkout}>Eliminar</button></div>}<button className="primary-button full" onClick={onClose}>Cerrar</button></div></div> }
